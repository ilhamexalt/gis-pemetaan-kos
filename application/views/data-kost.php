...
<body>
	<div class="page-header">
		<h1>Data kost</h1>
		<form action="<?= base_url('Admin/datakost') ?>" method="get">

			<input type="text" placeholder="Search.." style="border-radius: 5px; right:20px; top:25px; position: absolute;">
		</form>
	</div>
				...
				<div class="modal-content">
					...
					<form action="<?= base_url('Admin/hapuskost/' . $j['ID']); ?>" method="get">
						<div class="modal-body">
							Apakah yakin akan menghapus data kost <?= $j['name']; ?>?
						</div>
	...
	<?php foreach ($kost as $j) : ?>
		<div class="modal fade" id="editkost<?= $j['ID']; ?>" tabindex="-1" role="dialog" aria-labelledby="editkostLabel" aria-hidden="true">
			<form action="<?= base_url('Admin/editkost/' . $j['ID']); ?>" method="post">
				<div class="modal-dialog" role="document">
					<div class="modal-content" style="width: 600px;">
						...
						<div class="modal-body">
							...
							<div class="form-group">
								<input type="file" class="custom-file-input" id="image" name="image" size=20 value="<?= $j['image']; ?>" />
							</div>
						</div>
						<div class="modal-footer">
